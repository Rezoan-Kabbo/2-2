...
def make_signal(start_time, end_time, values):
    signal = DiscreteSignal(start_time, end_time) # By deafult calls __init__()

    if len(values) != len(signal): # Left side calls list's __len__(). Right side calls DiscreteSignal's __len__().
        raise ValueError("Incorrect number of samples")

    for i, value in enumerate(values): # for i, value in enumerate(values, start=1): for i=1 to start
        signal.set_value_at_time(start_time + i, value)
...
def signal_from_samples(start_time, end_time, samples):   # samples = {   0: 1,   2: 5 }
    signal = DiscreteSignal(start_time, end_time)      

    for t, value in samples.items(): # t->key , v->value
...
# To run the programme in terminal : 
# python main.py inputs/4.txt --out-dir outputs/4