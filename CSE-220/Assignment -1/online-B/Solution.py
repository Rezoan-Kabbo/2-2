...
def cascade(first_system, second_system, input_signal):
    ...
    intermediate_output = first_system.output(input_signal) # z = h2*x
    final_output = second_system.output(intermediate_output) # p = h1*z
...
def main():
    ...
    print()
    conclusion = "The cascade output equals x[n], so the cascade is an identity system and the differentiator and accumulator are inverse systems under zero initial conditions."
    print(conclusion)
